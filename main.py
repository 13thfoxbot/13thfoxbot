...
class Bot(commands.Bot):
    ...
    async def event_message(self, message: twitchio.Message):
        ...
        with open('shoutedoutList.txt', 'r') as f:
            solines = f.read()
            if message.author.name.lower() in self.whitelist and message.author.name.lower() not in solines:
    ...
    async def reset(self, ctx: commands.Context):
        if ctx.author.is_mod:
            ...
            self.shoutoutList = []
            open('shoutedoutList.txt', 'w').close()
    ...
    @routines.routine(time=datetime.datetime(year=2022, month=12, day=18, hour=12, minute=0))
    async def resetrout(self):
        # resets the shoutout list once a day
        print('shoutout list reset')
        # clear the shoutedoutList
        open('shoutedoutList.txt', 'w').close()
    ...
    async def send_reminder(self):
        ...
        if self.shoutoutList:
            ...
            with open('shoutedoutList.txt', 'a+') as f:
                f.write(f',{self.shoutoutList[0]}')
                solines = f.readlines()
                f.close()
            self.shoutoutList = self.shoutoutList[1:]
            print(f'routine shoutout {self.shoutoutList}')
            print(f'routine shoutedout {solines}')
